...
def main():
    from_date = os.environ.get("FROM_DATE", (datetime.now(timezone.utc) - timedelta(days=7)).strftime("%Y-%m-%d"))
    to_date = os.environ.get("TO_DATE", (datetime.now(timezone.utc) - timedelta(days=6)).strftime("%Y-%m-%d"))
    indicators = ["PhysicalFlow"]  # minimal required for ENTSOG
